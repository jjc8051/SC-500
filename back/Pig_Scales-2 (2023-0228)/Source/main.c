...
//
//	2023-0228	무게 측정 Data를 버퍼에 저장 하고 저장한 값을 (Data + NewData)/2 로 연산하여 연속적으로 무게 Data 를 평균 내도록 펌웨어 수정
//			스케일 조정 하는 부분도 (Data + NewData)/2 값을 대입하고 0값 이 들어오는 부분 안정화 작업 처리
//			측정무게 안정화 Count는 5번 에서 7번으로 늘려서 정확도를 약간 높임
//
//=================================================================================================
#include <main.h>
#include <avr/fuse.h>
...
//-------------------------------------------------------------------------------------------------
		...
		else if((Old_Value <= Value + (Admit_Weight*6)) & (Old_Value >= Value - (Admit_Weight*6))) {Room_Desplay();}	// 방 번호 표시
		Zero_Count = 0;
		CELL.Value = Old_Value;
		CELL.Old_Value = (Old_Value + Value)/2;				// 이전체중에 측정체중 저장
