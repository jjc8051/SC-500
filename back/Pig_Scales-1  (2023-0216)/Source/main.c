		...
		{								// 이전체중이 측정체중과 인증체중(50g)을 더한값과 같거나 작고 인증체중(50g)을 뺀값과 같거나 크면서 이전체중과 같으면
			...
			if(OK_Count == 3)					// 측정체중 OK Count 가 3보다 크면
			{
				if(!Zero_Buff)					// Zero_Buff 가 0 이면
				{
					...
					if     (Number == 1) {Data_1[1] = CELL.Value_1 = CELL.Value_1; CELL.Count_1++; Cell_Desplay(Number); Alarm(); CELL.Zero_Buff_1 = 0; Timer.Check_Limit_Timer_1 = 0;}
					else if(Number == 2) {Data_2[1] = CELL.Value_2 = CELL.Value_2; CELL.Count_2++; Cell_Desplay(Number); Alarm(); CELL.Zero_Buff_2 = 0; Timer.Check_Limit_Timer_2 = 0;}
					else if(Number == 3) {Data_3[1] = CELL.Value_3 = CELL.Value_3; CELL.Count_3++; Cell_Desplay(Number); Alarm(); CELL.Zero_Buff_3 = 0; Timer.Check_Limit_Timer_3 = 0;}
					// 1, 2, 3번 로드셀 이면 Data[1]에 측정체중 저장, 측정 횟수 증가, 체중표시, Zero_Buff 초기화
				}
				OK_Count = 0;					// 측정체중 OK Count 초기화
				printf("Value %ld \r\n", Value);
			}

			if     (Number == 1) {OK_Count_1 = OK_Count; CELL.Old_Value_1 = Old_Value; CELL.Value_1 = CELL.Value_1; Cell_Desplay(Number);}		// 1번 로드셀 이면 OK_Count 저장
			else if(Number == 2) {OK_Count_2 = OK_Count; CELL.Old_Value_2 = Old_Value; CELL.Value_2 = CELL.Value_2; Cell_Desplay(Number);}		// 2번 로드셀 이면 OK_Count 저장
			else if(Number == 3) {OK_Count_3 = OK_Count; CELL.Old_Value_3 = Old_Value; CELL.Value_3 = CELL.Value_3; Cell_Desplay(Number);}		// 3번 로드셀 이면 OK_Count 저장
		}
		else	// 이전체중이 측정체중과 인증체중(50g)을 더한값과 같거나 작고 인증체중(50g)을 뺀값과 같거나 크면서 이전체중과 같으면 이 아니면
		{
			Old_Value = Value;					// 이전체중에 측정체중 저장
			if     (Number == 1) {CELL.Old_Value_1 = Old_Value; CELL.Value_1 = CELL.Value_1; Cell_Desplay(Number); Zero_Count_1 = 0;}
			else if(Number == 2) {CELL.Old_Value_2 = Old_Value; CELL.Value_2 = CELL.Value_2; Cell_Desplay(Number); Zero_Count_2 = 0;}
			else if(Number == 3) {CELL.Old_Value_3 = Old_Value; CELL.Value_3 = CELL.Value_3; Cell_Desplay(Number); Zero_Count_3 = 0;}
