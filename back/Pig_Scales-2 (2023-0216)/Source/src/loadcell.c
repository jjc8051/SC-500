...
uint16_t Read_Load_Cell(void)							// 로드셀 측정값 읽기
{
	...
	uint16_t Re_cnt_1 = 100;						// 양수 연속 측정 후 초기화 횟수
	uint8_t Re_cnt_2 = 10;							// 음수 연속 측정 후 초기화 횟수
	...
	if(Value > 0)								// 무게 Data 가 있으면
	{
		...
		if(ValSum > 0)							// ValSum 이 0 보다 작으면
		{
			if(ValSum >= CELL.Old_ValSum)				// ValSum 이 Old_ValSum 보다크거나 같으면 
			{
				...
				{
					CELL.OFF_Set = OFF_SET + ValSum;	// 초기화 진행
					Alarm();				// 알람(비프 3번)
				}
			}
...
//-------------------------------------------------------------------------------------------------
		if(ValSum < -IN_Scales)						// ValSum 이 -입구체중 보다 작으면
		{
			Value_Count++;						// Value_Count 증가
		...
		if(IN_Scales > ValSum && ValSum > -IN_Scales)			// ValSum 이 입구체중 보다 작고 -입구체중 보다 크면
		{
			if     (ValSum > 0) {CELL.OFF_Set = OFF_SET + ValSum;}	// ValSum 이 0g 보다   크면 로드셀 이면 OFF_Set + ValSum
			else if(ValSum < 0) {CELL.OFF_Set = OFF_SET + ValSum;}	// ValSum 이 0g 보다 작으면 로드셀 이면 OFF_Set + ValSum
			ValSum = 0;						// ValSum 을 0으로 초기화
		}
...
void Cell_Zero_Set(void)							// 로드셀 0 값 찾기
{
	...
	Value = Load_Cell_Read();						// 로드셀 Data 읽기
	do{
...
void Interval_Set(void)								// Interval 설정
{
	...
	ee_write_word(Cell_Interval_Addr, INT);					// 로드셀 보정 값을 EEPROM 에 저장
}
...
//-------------------------------------------------------------------------------------------------
	...
	INT	  = ee_read_word(Cell_Interval_Addr);				// 로드셀 보정 값
//-------------------------------------------------------------------------------------------------
	CAPA_1000 = Cell_Capa / 1000;						// 로드셀 CAPA 1000의 자리
	CAPA_100  = Cell_Capa % 1000 / 100;					// 로드셀 CAPA  100의 자리
	CAPA_10   = Cell_Capa % 100 / 10;					// 로드셀 CAPA   10의 자리
