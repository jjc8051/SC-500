...
void Interval_Set(uint8_t Number)						// Interval 설정
{
	...
	while(SET.Menu && SET.Select)						// 메뉴가 1 이면 반복
	{
		...
		if     (Number == 1) {Interval_1 = INT; Value = CELL.Value_1 = Read_Load_Cell(Number);}	// 1번 로드셀 측정값
		else if(Number == 2) {Interval_2 = INT; Value = CELL.Value_2 = Read_Load_Cell(Number);}	// 2번 로드셀 측정값
		else if(Number == 3) {Interval_3 = INT; Value = CELL.Value_3 = Read_Load_Cell(Number);}	// 3번 로드셀 측정값

		if(Value)							// 측정체중이 있으면
		{
			if     (Number == 1) {CELL.Value_1 = CELL.Value_1 + Interval_1; INT_Desplay(Number);}	// 1번 로드셀 이면 측정체중 표시
			else if(Number == 2) {CELL.Value_2 = CELL.Value_2 + Interval_2; INT_Desplay(Number);}	// 2번 로드셀 이면 측정체중 표시
			else if(Number == 3) {CELL.Value_3 = CELL.Value_3 + Interval_3; INT_Desplay(Number);}	// 3번 로드셀 이면 측정체중 표시
		}
		else if(Value == 0) {INT_Desplay(Number);}			// 측정체중이 0 이면
		wdt_reset();							// 워치독 리셋
	}
	if     (Number == 1) {ee_write_word(Cell_Interval_1_Addr, INT);}	// 1번 로드셀 보정 값을 EEPROM 에 저장
	else if(Number == 2) {ee_write_word(Cell_Interval_2_Addr, INT);}	// 2번 로드셀 보정 값을 EEPROM 에 저장
	else if(Number == 3) {ee_write_word(Cell_Interval_3_Addr, INT);}	// 3번 로드셀 보정 값을 EEPROM 에 저장
}
