...
void Key_Scan(void)								// key 입력 체크
{
	if(!KEY.Buff)								// Key 입력이 없었으면
	{
		...
		{
			if(!SET.NumSET) {SET.NumSET = 1;}			// NumSET모드 = 1 아니면 NumSET모드 설정
		}
...
//		방 번호 값
//-------------------------------------------------------------------------------------------------
	SAVE.RoomNum = ee_read_byte(Cell_ROOM_Addr);				// 방번호 설정값 불러오기
	if(SAVE.RoomNum > 100 || SAVE.RoomNum <= 0)				// 방번호 설정값이 100보다 크거나 0과 같거나 작으면
	{
		SAVE.RoomNum = 1;						// 방번호 설정값을 1 으로 초기화
		ee_write_byte(Cell_ROOM_Addr, SAVE.RoomNum);			// 방번호 설정값을 EEPROM 에 저장
	}
//-------------------------------------------------------------------------------------------------
//		스톨 번호 값
//-------------------------------------------------------------------------------------------------
	SAVE.StalNum = ee_read_byte(Cell_STAL_Addr);				// 스톨번호 설정 불러오기
	if(SAVE.StalNum > 100 || SAVE.StalNum <= 0)				// 스톨번호 설정값이 100보다 크거나 0과 같거나 작으면
	{
		SAVE.StalNum = 1;						// 스톨번호 설정값을 1 으로 초기화
		ee_write_byte(Cell_STAL_Addr, SAVE.StalNum);			// 스톨번호 설정값을 EEPROM 에 저장
	}
//-------------------------------------------------------------------------------------------------
//		자동모드 설정 값
//-------------------------------------------------------------------------------------------------
...
void Seting_Mode_1(void)							// 셋팅모드 1
{
	...
	SET.NumSET = 0;
	ee_write_byte(Cell_ROOM_Addr, SAVE.RoomNum);
	ee_write_byte(Cell_STAL_Addr, SAVE.StalNum);
	Alarm();
}
	...
	if(Timer.RTC_Timer >= 1000)						// 5mSec X 1000 = 5Sec 이상이 되면 진입
	{
		...
		if(RTC_T.OLD_D != RTC_T.Date)					// 날짜 Data 가 다르면
		{
			...
			ee_write_byte (OLD_Date_Addr, RTC_T.OLD_D);		// 날짜 를 EEPROM 에 저장
 			...
 			SAVE.Count = 0;						// 측정 무게 Count Clear
		}
	}
}
...
//-------------------------------------------------------------------------------------------------
	while(1)
	{
		...
		wdt_reset();							// Reset WDT
		_delay_ms(100);							// 연속 동작을 막기위한 Delay
	}
}
