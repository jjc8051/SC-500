...
void Key_Scan(void)								// key 입력 체크
{
	if(!KEY.Buff)								// Key 입력이 없었으면
	{
		...
		if(!SET.Menu && KEY.Key == 2)					// 메뉴모드가 아니고 선택 Key 입력이 있으면
		{
			if(SET.Save) {SET.Save = 0;}				// 저장모드 = 1 이면 저장모드 해제
			else                                                    // 저장모드 = 0 이면 저장모드 설정
			{
				SET.Save = 1;					
				DIGIT[2][3] = FND_MINUS;				// - 표시
				DIGIT[2][2] = FND_MINUS;
				DIGIT[2][1] = FND_MINUS;
				DIGIT[2][0] = FND_MINUS;
			}				
...
//  		if((Timer.Limit_Timer >= 500) & ((Value <= Old_Value + Admit_Weight) & (Value >= Old_Value - Admit_Weight)))
		...
		{								// 이전체중이 측정체중과 인증체중(500g)을 더한값과 같거나 작고 인증체중(500g)을 뺀값과 같거나 크면서 이전체중과 같으면
			...
			{
				FND_Value_Data(1, 1, 10);			// FND 1 FND_MINUS
				...
				FND_Value_Data(2, 2, 12);			// FND 2 FND_OFF
				printf("Value %ld \r\n", Value + Interval);
			}
		...
		else	// 이전체중이 측정체중과 인증체중(500g)을 더한값과 같거나 작고 인증체중(500g)을 뺀값과 같거나 크면서 이전체중과 같지 않으면
		{
			...
			OK_Count = 0;						// 측정체중 OK Count 초기화
			FND_Value_Data(1, 1, 12);				// FND 1 FND_OFF
			Cell_Desplay();
		}
	}
	else 									// 측정체중이 없면
...
// 	if      (commCnt > (RECORD.Total_Cnt % 300) && (RECORD.Total_Cnt < 300)) {commCnt = 0;}
// 	else if (commCnt > 299) {commCnt = 0;}
	if(CommCnt > 299) {CommCnt = 0;}
...
//-------------------------------------------------------------------------------------------------
	...
	SerialData[cnt++] = SORTING_FRAGEMNT;					// 21 : 장비기종(포유자돈, 비육돈)
	...
	SerialData[cnt++] = SAVE.Count / 256;					// 측정무게 CommCnt 상위				// 10
	SerialData[cnt++] = SAVE.Count % 256;					// 측정무게 CommCnt 하위
	...
	SerialData[cnt++] = SAVE.Year[CommCnt];					// 측정 연도
	SerialData[cnt++] = SAVE.Month[CommCnt];				// 측정 월
	SerialData[cnt++] = SAVE.Date[CommCnt];					// 측정 일
	SerialData[cnt++] = SAVE.Hour[CommCnt];					// 측정 시
	SerialData[cnt++] = SAVE.Min[CommCnt];					// 측정 분					// 20
	SerialData[cnt++] = SAVE.Sec[CommCnt];					// 측정 초

	SerialData[cnt++] = SAVE.SandCnt;					// Data 순번
