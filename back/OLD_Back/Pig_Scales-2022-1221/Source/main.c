...
void FND_ALL_OFF(void)								// FND 전체 OFF
{
	...
	{
		FND_Value_Data(i, i, 10);					// FND 1 FND_OFF 표시
	}
}
...
// 			if(KEY.Count > 5) {KEY.Count = 0;}			// Key 가 눌린횟수가 5보다 크면 0으로 초기화
		...
		if(!SET.Menu && !SET.Room && KEY.Key == 2)			// 메뉴모드 와 SET.Room 가 1이 아니고 선택 Key 입력이 있으면
		{
			if(SET.Save) {SET.Save = 0;}				// 저장모드 = 1 이면 저장모드 해제
			else                                                    // 저장모드 = 0 이면 저장모드 설정
			{
				SET.Save = 1;
				FND_Value_Data(2, 2, 10);			// FND 1 FND_MINUS 표시
			}
		}

		if(!SET.Menu && KEY.Key == 1)					// 메뉴모드가 아니고 선택 Key 입력이 있으면
		{
			if(!SET.Room) {SET.Room = 1;}				// ROOM모드 = 1 아니면 ROOM모드 설정
		}
...
void Seting_Mode_1(void)							// 셋팅모드 1
{
	...
	while(cnt != 2)								// 메뉴 와 선택 이 모두 1 이면 반복
	{
		if(cnt == 0)
		{
			if(KEY.Buff)						// Key 입력이 있으면
			{
				if     (KEY.Key == 1 && SAVE.RoomNum < 100) {SAVE.RoomNum++; KEY.Key = 0;}	// 업  버튼 이면 갯수 설정값 증가
				else if(KEY.Key == 3 && SAVE.RoomNum > 1)   {SAVE.RoomNum--; KEY.Key = 0;}	// 다운버튼 이면 갯수 설정값 감소
			...
			{
				DIGIT[1][3] = FND_OFF;				// FND 1 에  번호 표시
				DIGIT[1][2] = fnd_tbl[SAVE.RoomNum / 100];
				DIGIT[1][1] = fnd_tbl[SAVE.RoomNum % 100 / 10];
				DIGIT[1][0] = fnd_tbl[SAVE.RoomNum % 10];
			}
			DIGIT[2][3] = FND_OFF;					// FND 2 에 스톨 번호 표시
			DIGIT[2][2] = fnd_tbl[SAVE.StalNum / 100];
			DIGIT[2][1] = fnd_tbl[SAVE.StalNum % 100 / 10];
			DIGIT[2][0] = fnd_tbl[SAVE.StalNum % 10];
		}
		else if(cnt == 1)
		{
			if(KEY.Buff)						// Key 입력이 있으면
			{
				if     (KEY.Key == 1 && SAVE.StalNum < 100) {SAVE.StalNum++; KEY.Key = 0;}	// 업  버튼 이면 갯수 설정값 증가
				else if(KEY.Key == 3 && SAVE.StalNum > 1)   {SAVE.StalNum--; KEY.Key = 0;}	// 다운버튼 이면 갯수 설정값 감소
				else if(KEY.Key == 2)                       {cnt++; SET.Save = 0; KEY.Key = 0;}	// 선택버튼 이면 SET.Room 초기화
			}
...
//=================================================================================================
void Room_Desplay(void)								// 방, 스톨 번호 표시
{
	DIGIT[1][3] = FND_OFF;							// FND 1 에 방 번호 표시
	DIGIT[1][2] = fnd_tbl[SAVE.RoomNum / 100];
	DIGIT[1][1] = fnd_tbl[SAVE.RoomNum % 100 / 10];
...
void Stal_Desplay(void)								// 방, 스톨 번호 표시
{
	DIGIT[2][3] = FND_OFF;							// FND 2 에 스톨 번호 표시
	DIGIT[2][2] = fnd_tbl[SAVE.StalNum / 100];
	DIGIT[2][1] = fnd_tbl[SAVE.StalNum % 100 / 10];
...
//-------------------------------------------------------------------------------------------------
	if(Value)								// 측정체중이 있으면
	{
//  		if((Timer.Limit_Timer >= 500) & ((Value <= Old_Value + Admit_Weight) & (Value >= Old_Value - Admit_Weight)))
 		if((Value <= Old_Value + Admit_Weight) & (Value >= Old_Value - Admit_Weight))
		{								// 이전체중이 측정체중과 인증체중(500g)을 더한값과 같거나 작고 인증체중(500g)을 뺀값과 같거나 크면서 이전체중과 같으면
			OK_Count++;						// 측정체중 OK Count 증가
			if(OK_Count > 3)					// 측정체중 OK Count 가 3보다 크면
			{
				FND_Value_Data(1, 1, 10);			// FND 1 FND_MINUS 표시

 				if(SET.Save)					// 저장 Key 가 눌렸으면
 				{
					if(SAVE.Count < 299) {SAVE.Count++;}	// 저장횟수 가 0 ~ 299(300개)보다 작으면 저장횟수 증가
					else                 {SAVE.Count = 0;}	// 아니면 저장횟수 증가

 	 				SAVE.Data [SAVE.Count] = CELL.Value = Value + Interval;
 	 				SAVE.Year [SAVE.Count] = RTC_Single_Read(RTC_YEAR);	// YEAR
 	 				SAVE.Month[SAVE.Count] = RTC_Single_Read(RTC_MONTH);	// MONTH
 	 				SAVE.Date [SAVE.Count] = RTC_Single_Read(RTC_DATE);	// DATE
 	 				SAVE.Hour [SAVE.Count] = RTC_Single_Read(RTC_HOUR);	// HOUR
 	 				SAVE.Min  [SAVE.Count] = RTC_Single_Read(RTC_MINUTE);	// MINUTE
 	 				SAVE.Sec  [SAVE.Count] = RTC_Single_Read(RTC_SECOND);	// SECOND
		...
		else	// 이전체중이 측정체중과 인증체중(500g)을 더한값과 같거나 작고 인증체중(500g)을 뺀값과 같거나 크면서 이전체중과 같지 않으면
		{
...
// 	GetWeightHistoryRecord_EEPROM(commCnt++);

	if(CommCnt < SAVE.Count) {CommCnt++;}
	else                     {CommCnt = 0;}
...
//-------------------------------------------------------------------------------------------------
	...
	SerialData[cnt++] = CD_STATUS;						// 0 : 구분코드 (실시간상태 정보) 
	...
	SerialData[cnt++] = SAVE.Count / 256;					// 측정횟수 상위				// 10
	SerialData[cnt++] = SAVE.Count % 256;					// 측정횟수 하위
	SerialData[cnt++] = SAVE.Data[CommCnt] / 256;				// 측정무게 [CommCnt] 상위
	SerialData[cnt++] = SAVE.Data[CommCnt] % 256;				// 측정무게 [CommCnt] 하위

	SerialData[cnt++] = SAVE.RoomNum;					// 방 번호
	SerialData[cnt++] = SAVE.StalNum;					// 스톨 번호				// 15

	SerialData[cnt++] = SAVE.Year[CommCnt];					// 측정 년 [CommCnt]
	SerialData[cnt++] = SAVE.Month[CommCnt];				// 측정 월 [CommCnt]
	SerialData[cnt++] = SAVE.Date[CommCnt];					// 측정 일 [CommCnt]
	SerialData[cnt++] = SAVE.Hour[CommCnt];					// 측정 시 [CommCnt]
	SerialData[cnt++] = SAVE.Min[CommCnt];					// 측정 분 [CommCnt]			// 20
	SerialData[cnt++] = SAVE.Sec[CommCnt];					// 측정 초 [CommCnt]

	SerialData[cnt++] = CommCnt / 256;					// Data 순번 상위
	SerialData[cnt++] = CommCnt % 256;					// Data 순번 하위

	SerialData[cnt++] = 0x00;						// 예비
	SerialData[cnt++] = 0x00;						// 예비					// 25
	SerialData[cnt++] = 0x00;						// 예비
	SerialData[cnt++] = 0x00;						// 예비

	for(uint8_t icnt2 = 1; icnt2 < cnt; icnt2++) {ChkSum += SerialData[icnt2];}	// 체크썸 계산
	SerialData[cnt++] = ChkSum;						// 체크썸				// 28
	SerialData[cnt++] = ETX_Iontec;						// ETX_Iontec : 0xF1			// 29
//-------------------------------------------------------------------------------------------------
//		Serial Protocol End
//-------------------------------------------------------------------------------------------------
...
// 					Send_To_Server(Cell_ID);		// 무게 Data PC로 전송
...
// 			memset(SAVE.Data, 0, 300);				// 측정 무게 Data  Clear
// 			SAVE.DataCount = 0;					// 측정 무게 Count Clear
		}
	}
}
...
//-------------------------------------------------------------------------------------------------
	while(1)
	{
		if(SET.Menu) {Seting_Mode_0();}					// SET Menu 가 설정되어 있으면 Seting_Mode   진입
		if(SET.Room) {Seting_Mode_1();}					// SET.Room 가 설정되어 있으면 ROOM_Set_Mode 진입
		Check_Value();
