...
//-------------------------------------------------------------------------------------------------
//		printf("ValSum %ld \r\n", ValSum);
 		if(ValSum < 0 || ValSum > 10000000)				// ValSum 이 0 보다 작거나 1Ton 보다 크면 최기화 카운터 
		{
			Value_Count++;						// Value_Count 증가
			if(Value_Count > Set_cnt)				// Value_Count 가 초기화 값 보다 크면
			{
 		...
 		if(ValSum > 0 && IN_Scales > ValSum)				// ValSum 이 0 크고 입구체중 보다 작으면
 		{
			Value_Count++;						// Value_Count 증가
			if(Value_Count > Set_cnt)				// Value_Count 가 초기화 값 보다 크면
			{
	 			CELL.OFF_Set = OFF_SET + ValSum;				// OFF_Set + 25
			}
			ValSum = 0;						// ValSum 을 0으로 초기화
 		}
