...
void Cell_Desplay(void)								// 로드셀 무게 표시
{
	...
	Weight_1     = CELL.Value % 10 / 1;					// 측정무게   0.01의 자리   0.01Kg
...
//=================================================================================================
void INT_Desplay(void)								// 보정값 무게 표시
{
	Weight_10000 = CELL.Value / 10000;					// 측정무게 100.00의 자리 100.00kg
	Weight_1000  = CELL.Value % 10000 / 1000;				// 측정무게  10.00의 자리  10.00kg
	Weight_100   = CELL.Value % 1000 / 100;					// 측정무게   1.00의 자리   1.00kg
...
//=================================================================================================
int32_t Load_Cell_Read(void)							// 로드셀 Data 읽기, HX711 제어
{
	...
	if(Value > 0x7FF000) {Value = 0;}					// 전체 Data 가 상한값 이상이면 Data 초기화
	Value = Value / 1000;							// 낮은 Data 값 지우기
	Value = Value * 1000;							// 낮은 Data 값 지우기
	Value = Value / CELL.Proportion;					// Data / 비율 = 무게
...
uint16_t Read_Load_Cell(void)							// 로드셀 측정값 읽기
{
	...
	uint8_t Re_cnt_2 = 5;							// 음수 연속 측정 후 초기화 횟수
	uint32_t CAPA;								// Cell_Capa * 1000
// 	OFF_SET = 0;								// OFF_SET 값 저장용 변수 초기화
//-------------------------------------------------------------------------------------------------
	Value = Load_Cell_Read();						// 로드셀 측정값 읽기
// 	OFF_SET = CELL.OFF_Set;							// 로드셀 OFF_Set
	IN_S_Value = IN_Scales;							// 로드셀 입구체중
	CAPA = Cell_Capa * 1000;
//-------------------------------------------------------------------------------------------------
	if(Value > 0)								// 무게 Data 가 있으면
	{
		...
		{
			if(ValSum >= CELL.Old_ValSum)				// ValSum 이 Old_ValSum 보다크거나 같으면 
			{
				Value_Count++;					// Value_Count 증가
				if(Value_Count > Re_cnt_1)			// 초기화 값 보다 크면
				{
					CELL.OFF_Set = CELL.OFF_Set + ValSum;	// 초기화 진행
					Alarm();				// 알람(비프 3번)	
				}
			}	
			else {CELL.Old_ValSum = ValSum;}			// Old_ValSum 을 ValSum 로 저장
		}
...
//-------------------------------------------------------------------------------------------------
			Value_Count++;						// Value_Count 증가
			if(Value_Count > Re_cnt_2)				// Value_Count 가 초기화 값 보다 크면
			{
				Cell_Zero_Set(1);
				Cell_Zero_Set(1);
			}
		...
		if(IN_S_Value > ValSum && ValSum > -IN_S_Value)			// ValSum 이 입구체중 보다 작고 -입구체중 보다 크면
		{
			...
			{
				CELL.OFF_Set = CELL.OFF_Set + ValSum;		// 로드셀 이면 OFF_Set + ValSum
			}
...
void Cell_Zero_Set(uint8_t Number)						// 로드셀 0 값 찾기
{
	...
	{
		Value = Load_Cell_Read();						// 1번 로드셀 Data 읽기
		do{
			Value_Sum = CELL.OFF_Set - Value;			// 1번 로드셀 OFF_Set 초기값(20Kg) - Data = Value_Sum
...
void IN_Scales_Set(void)							// IN_Scales 설정
{
	...
	IN_Scales = ee_read_word(Cell_IN_S_Addr);					// 로드셀 이면 입구체중값 불러오기
//-------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------
	ee_write_word(Cell_IN_S_Addr, IN_Scales);					// 로드셀 이면 입구체중값 저장
...
void RO_Set(void)								// RO 설정
{
	...
	RO = ee_read_dword(Cell_RO_Addr);					// 로드셀 이면 RO값 불러오기
//-------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------
	ee_write_dword(Cell_RO_Addr, RO);					// 로드셀 이면 정격출력값 저장
...
void Interval_Set(void)								// Interval 설정
{
	...
	while(SET.Menu && SET.Select)						// 메뉴가 1 이면 반복
	{
		...
		if(KEY.Buff)							// Key 입력이 있으면
		{
			if     (KEY.Key == 1) {INT++; KEY.Key = 0;}		// 업  버튼 이면 RO값 1의 자리 증가
			else if(KEY.Key == 3) {INT--; KEY.Key = 0;}		// 다운버튼 이면 RO값 1의 자리 감소

			if(INT >= 0)
			{
				DIGIT[1][3] = FND_OFF;				// 로드셀 보정 100의 자리
				DIGIT[1][2] = fnd_tbl[INT / 100];		// 로드셀 보정 100의 자리
				DIGIT[1][1] = fnd_tbl[INT % 100 / 10 ];		// 로드셀 보정  10의 자리
				DIGIT[1][0] = fnd_tbl[INT % 10 ];		// 로드셀 보정   1의 자리
			}
			else
			{
				cnt = ~INT + 1;
				DIGIT[1][3] = FND_MINUS;			// 로드셀 보정 -      자리
				DIGIT[1][2] = fnd_tbl[cnt / 100];		// 로드셀 보정  100의 자리
				DIGIT[1][1] = fnd_tbl[cnt % 100 / 10 ];		// 로드셀 보정   10의 자리
				DIGIT[1][0] = fnd_tbl[cnt %10 ];		// 로드셀 보정    1의 자리
			}
		}
		...
		else if(Value == 0) {INT_Desplay();}				// 측정체중이 0 이면
		wdt_reset();							// 워치독 리셋
	}
...
//-------------------------------------------------------------------------------------------------
	...
	Interval  = ee_read_word(Cell_Interval_Addr);				// 로드셀 보정 값
//-------------------------------------------------------------------------------------------------
	CAPA_1000 = Cell_Capa / 1000;						// 로드셀 CAPA 1000의 자리
	CAPA_100  = Cell_Capa % 1000 / 100;					// 로드셀 CAPA  100의 자리
	CAPA_10   = Cell_Capa % 100 / 10;					// 로드셀 CAPA   10의 자리
	...
	while(SET.Menu)								// 메뉴가 1 이면 반복
	{
		...
		switch(Count)							// Count
		{
			...
			case 6:							// Count = 6 이면
				...
				{
					DIGIT[1][3] = FND_OFF;				// 로드셀 보정 100의 자리
					DIGIT[1][2] = fnd_tbl[Interval / 100];		// 로드셀 보정 100의 자리
					DIGIT[1][1] = fnd_tbl[Interval % 100 / 10];	// 로드셀 보정  10의 자리
					DIGIT[1][0] = fnd_tbl[Interval % 10 ];		// 로드셀 보정   1의 자리
				}
				else
				{
					cnt = ~Interval + 1;
					DIGIT[1][3] = FND_MINUS;			// 로드셀 보정 100의 자리
					DIGIT[1][2] = fnd_tbl[cnt / 100];		// 로드셀 보정 100의 자리
					DIGIT[1][1] = fnd_tbl[cnt % 100 / 10];		// 로드셀 보정  10의 자리
					DIGIT[1][0] = fnd_tbl[cnt %10 ];		// 로드셀 보정   1의 자리
				}
	...
	while(SET.Menu)								// 메뉴가 1 이면 반복
	{
		...
		switch(Count)							// Count
		{
			...
			case 1:							// Count = 1 이면
				...
				if(SET.Select) {Cell_Select();}			// 1번 로드셀 설정 선택
			break;
			...
			case 2:							// Count = 2 이면
				...
				if(SET.Select) {Cell_Select();}			// 2번 로드셀 설정 선택
			break;
			...
			case 3:							// Count = 3 이면
				...
				if(SET.Select) {Cell_Select();}			// 3번 로드셀 설정 선택
			break;
