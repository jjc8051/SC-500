...
//
//	2023-0302	측정무게 안정화 Count 를 최소로 줄여서 안정화 보다 측정 속도를 높이는 것을 선택
//
//=================================================================================================
#include <main.h>
#include <avr/fuse.h>
...
//-------------------------------------------------------------------------------------------------
		...
		{
			if((CELL.Old_Value_1 <= Value + (Admit_Weight * 5)) && (CELL.Old_Value_1 >= Value - (Admit_Weight * 5)))
			{
				CELL.Old_Value = (CELL.Old_Value_1 + Value)/2;	// 이전체중에 측정체중 저장
				CELL.Value = CELL.Old_Value;
			}
			else
			{
// 				CELL.Value = CELL.Old_Value_1 = Value;
