...
<<<<<<< HEAD:back/Pig_Scales-1  (2023-0106)/Source/include/typedefine.h
	volatile uint16_t Check_Limit_Timer_1;			// 무게측정 제한 시간 Timer 1
	volatile uint16_t Check_Limit_Timer_2;			// 무게측정 제한 시간 Timer 2
	volatile uint16_t Check_Limit_Timer_3;			// 무게측정 제한 시간 Timer 3

=======
>>>>>>> 37084d9a3f556cf7c84333cf20a704335f1b4c28:back/Pig_Scales-2 (2023-0302)/Source/include/typedefine.h
} _TIMER;
...
{
	volatile double Proportion_1;				// 1번 로드셀 비율
	volatile double Proportion_2;				// 2번 로드셀 비율
	volatile double Proportion_3;				// 3번 로드셀 비율

<<<<<<< HEAD:back/Pig_Scales-1  (2023-0106)/Source/include/typedefine.h
	volatile int32_t Value_1;				// 1번 로드셀 측정 값
	volatile int32_t Value_2;				// 2번 로드셀 측정 값
	volatile int32_t Value_3;				// 3번 로드셀 측정 값

	volatile int32_t Old_Value_1;				// 1번 이전 측정 값
	volatile int32_t Old_Value_2;				// 2번 이전 측정 값
	volatile int32_t Old_Value_3;				// 3번 이전 측정 값

	volatile int32_t Old_ValSum_1;				// 1번 이전 측정 값
	volatile int32_t Old_ValSum_2;				// 2번 이전 측정 값
	volatile int32_t Old_ValSum_3;				// 3번 이전 측정 값

	volatile int32_t OFF_Set;				// 범용 OFF_Set(입력값을 0값 으로 만들기위한 값)
	volatile int32_t OFF_Set_1;				// 1번 로드셀 OFF_Set(입력값을 0값 으로 만들기위한 값)
	volatile int32_t OFF_Set_2;				// 2번 로드셀 OFF_Set(입력값을 0값 으로 만들기위한 값)
	volatile int32_t OFF_Set_3;				// 3번 로드셀 OFF_Set(입력값을 0값 으로 만들기위한 값)

	volatile uint16_t Count_1;				// 1번 로드셀 측정 횟수
	volatile uint16_t Count_2;				// 2번 로드셀 측정 횟수
	volatile uint16_t Count_3;				// 3번 로드셀 측정 횟수

	volatile int8_t Zero_Buff_1;				// 1번 측정 값 Zero Buff
	volatile int8_t Zero_Buff_2;				// 2번 측정 값 Zero Buff
	volatile int8_t Zero_Buff_3;				// 3번 측정 값 Zero Buff

} _RoadCell;
_RoadCell CELL;							// CELL 구조체명 선언

uint16_t Data_1[20];						// 1번 로드셀 측정 값 버퍼 20개
uint16_t Data_2[20];						// 2번 로드셀 측정 값 버퍼 20개
uint16_t Data_3[20];						// 3번 로드셀 측정 값 버퍼 20개
=======
	volatile int32_t Value;					// 로드셀 측정 값
	volatile int32_t OFF_Set;				// 로드셀 OFF_Set(입력값을 0값 으로 만들기위한 값)
	volatile int32_t Old_Value;				// 이전 측정 값

	volatile int32_t Old_Value_1;				// 이전 측정 값
