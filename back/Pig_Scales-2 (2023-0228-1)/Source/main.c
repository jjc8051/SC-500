//=================================================================================================
//		ProductName 	:	Pig_Scales V4.4 SC-500
//=================================================================================================
//		File  Name	:	main.c
...
void Check_Value(void)								// 로드셀 체중측정
{
	...
	uint32_t Old_Value = (CELL.Old_Value_0 + CELL.Old_Value_1 + CELL.Old_Value_2 + CELL.Old_Value_3 + CELL.Old_Value_4
			    + CELL.Old_Value_5 + CELL.Old_Value_6 + CELL.Old_Value_7 + CELL.Old_Value_8 + CELL.Old_Value_9)/10;	// 이전측정 무게 5개 를 더해서 평균을 냄
	uint8_t cnt = 2;							// OK_Count 갯수

//-------------------------------------------------------------------------------------------------
	if(Value)								// 측정체중이 있으면
	{
// 		if((Value <= Old_Value + Admit_Weight) & (Value >= Old_Value - Admit_Weight))	// 이전체중이 측정체중과 인증체중(10x10g)을 더한값과 같거나 작고 인증체중(10x10g)을 뺀값과 같거나 크면서 이전체중과 같으면
		if((Old_Value <= Value + Admit_Weight) & (Old_Value >= Value - Admit_Weight))	// 이전체중이 측정체중과 인증체중(10x10g)을 더한값과 같거나 작고 인증체중(10x10g)을 뺀값과 같거나 크면서 이전체중과 같으면
		{
//			printf("Value %ld \r\n", Value);
			Sec_Count = 0;
			if(SET.Save || Auto_Mode) {OK_Count++;}			// 자동 모드일때 측정체중 OK Count 증가
			if(OK_Count == cnt)					// 측정체중 OK Count 가 5보다 크면
			{
...
// 					SAVE.Data   [Data_Number] = CELL.Value = Value;
					SAVE.Data   [Data_Number] = CELL.Value = Old_Value;
...
// 					ee_write_word(MEASURE_WEIGHT + (MEASURE_LENGTH * EepCnt), CELL.Value = Value);
					ee_write_word(MEASURE_WEIGHT + (MEASURE_LENGTH * EepCnt), CELL.Value = Old_Value);
...
// 		else if((Value <= Old_Value + 150) & (Value >= Old_Value - 150))	// 이전체중이 측정체중과 인증체중(100x10g)을 더한값과 같거나 작고 인증체중(100x10g)을 뺀값과 같거나 크면서 이전체중과 같으면
		else if((Old_Value <= Value + 150) & (Old_Value >= Value - 150))	// 이전체중이 측정체중과 인증체중(100x10g)을 더한값과 같거나 작고 인증체중(100x10g)을 뺀값과 같거나 크면서 이전체중과 같으면
		{
// 			CELL.Value = Value;
			CELL.Value = Old_Value;
		...
		CELL.Old_Value_9 = CELL.Old_Value_8;
		CELL.Old_Value_8 = CELL.Old_Value_7;
		CELL.Old_Value_7 = CELL.Old_Value_6;
		CELL.Old_Value_6 = CELL.Old_Value_5;
		CELL.Old_Value_5 = CELL.Old_Value_4;
		CELL.Old_Value_4 = CELL.Old_Value_3;
		CELL.Old_Value_3 = CELL.Old_Value_2;
		CELL.Old_Value_2 = CELL.Old_Value_1;
		CELL.Old_Value_1 = CELL.Old_Value_0;
		CELL.Old_Value_0 = Value;
	}
	else									// 측정체중이 없면
	{
		...
		{
			CELL.Old_Value_0 = CELL.Old_Value_1 = CELL.Old_Value_2 = CELL.Old_Value_3 = CELL.Old_Value_4 = 0;	// 이전측정체중 초기화
			CELL.Old_Value_5 = CELL.Old_Value_6 = CELL.Old_Value_7 = CELL.Old_Value_8 = CELL.Old_Value_9 = 0;	// 이전측정체중 초기화
...
void Time_Check(void)								// 시간 체크
{
	if(Timer.RTC_Timer >= 1000)						// 5mSec X 1000 = 5Sec 이상이 되면 진입
	{
		...
		if(Sec_Count > 10) {Sec_Count = 10;}				// Sec_Count 가 10 보다 크면 10 으로 유지
	}
}
...
//-------------------------------------------------------------------------------------------------
	while(1)
	{
// 		if(Sec_Count < 6) {Check_Value();}				// 5Sec x 6 = 30Sec 보다 크면 FND ALL OFF
// 		else
// 		{
// 			while(!KEY.Buff)
// 			{
