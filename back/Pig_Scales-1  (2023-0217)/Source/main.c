...
//
//	2022-1212	측정체중이 이전체중 + 인증체중(100g)값 보다 작거나 같으면서 이전체중 - 인증체중(100g)값 보다 크거나 같으면 무게측정 OK
//			연속 무게측정 3회 이상 이면 즉정 무게 Data 저장
//
//	2022-1216	날짜 변경에의한 카운터 초기화 오류 수정
//			구조체에서 int 로 형변환에 에러가 발생
//			같은 구조체로 변경하여 처리
//
//	2023-0103	실시간 정보요청 코드를 1에서 0으로 처리(통신이 되지 않았음)
//	2023-0106	파일 폴더명 변경
//	
//	2023-0216	노이즈에 의한 튀는 현상도 너무 많고, 정상 적인 무게를 인식 하지 못하는 현상 발생
//			이를 해결 하기 위해 노이즈를 최대한 제거하기 위해 입력받는 부분에대한 소프트웨어 필터링 작업
//			펌웨어에 의한 필터링으로 어느 정도 안정된값을 받을수 있도록 처리
//			Pig_Scales-2 의 안정화되어진 펌웨어 이식
//			배율계산시에서 입력 전압 부분을 4V로 고정하여 계산 하도록 변경
//			아날로그 그라운드 단자 (AGND) 는 System GND 와연결 (노이즈 현격하게 감소)
//
//	2023-0217	Data 밀어내기(순번), 내림 차순에서 올림 차순으로 (최신값을 제일 마지막 번호로) 정리  
//			펌웨어 버젼 정리, 펌웨어 백업 폴더 정리
//
//=================================================================================================
#include <main.h>
#include <avr/fuse.h>
...
//-------------------------------------------------------------------------------------------------
	...
	{
		Cell_ID_2 = 2;							// 2번 로드셀 ID 값을 2로 초기화
	...
	{
		Cell_ID_3 = 3;							// 3번 로드셀 ID 값을 3로 초기화
...
//-------------------------------------------------------------------------------------------------
	...
	{
		Cell_Capa_1 = 200;						// 1번 로드셀 CAPA 값을 100kg로 초기화, 곱하기 1,000으로 계산 100kg = 100000g
	...
	{
		Cell_Capa_2 = 200;						// 2번 로드셀 CAPA 값을 100kg로 초기화, 곱하기 1,000으로 계산 100kg = 100000g
	...
	{
		Cell_Capa_3 = 200;						// 3번 로드셀 CAPA 값을 100kg로 초기화, 곱하기 1,000으로 계산 100kg = 100000g
...
//-------------------------------------------------------------------------------------------------
	if(Value)								// 측정체중이 있으면
	{
		...
		{								// 이전체중이 측정체중과 인증체중(50g)을 더한값과 같거나 작고 인증체중(50g)을 뺀값과 같거나 크면서 이전체중과 같으면
			...
			if(OK_Count == 3)					// 측정체중 OK Count 가 3보다 크면
			{
				if(!Zero_Buff)					// Zero_Buff 가 0 이면
				{
					...
					if (Number == 1)												// 1번 로드셀 이면
					{
						CELL.Count_1++;												// 1번 로드셀 측정 횟수 증가
						if(CELL.Count_1 < 20) {Data_1[CELL.Count_1] = CELL.Value_1 = Value;}					// 측정 횟수 가 20보다 작으면 버퍼에 카운터 듣 적용
						else {Data_1[21] = CELL.Value_1 = Value; for(uint8_t i = 1; i < 21; i++) {Data_1[i] = Data_1[i+1];}}	// 측정 횟수 가 20보다 크면 버퍼를 1칸씩 니려서 저장
						Cell_Desplay(Number);											// 로드셀 무게표시
						Alarm();												// 알람
						CELL.Zero_Buff_1 = 0;											// 로드셀 Zero_Buff_1 초기화
						Timer.Check_Limit_Timer_1 = 0;										// 무게측정 제한 시간 초기화 
					}
					else if(Number == 2)												// 2번 로드셀 이면
					{
						CELL.Count_2++;												// 2번 로드셀 측정 횟수 증가
						if(CELL.Count_2 < 20) {Data_2[CELL.Count_2] = CELL.Value_2 = Value;}					// 측정 횟수 가 20보다 작으면 버퍼에 카운터 듣 적용
						else {Data_2[21] = CELL.Value_2 = Value; for(uint8_t i = 1; i < 21; i++) {Data_2[i] = Data_2[i+1];}}	// 측정 횟수 가 20보다 크면 버퍼를 1칸씩 니려서 저장
						Cell_Desplay(Number);											// 로드셀 무게표시
						Alarm();												// 알람
						CELL.Zero_Buff_2 = 0;											// 로드셀 Zero_Buff_1 초기화
						Timer.Check_Limit_Timer_2 = 0;										// 무게측정 제한 시간 초기화 
					}
					else if(Number == 3)												// 3번 로드셀 이면
					{
						CELL.Count_3++;												// 3번 로드셀 측정 횟수 증가
						if(CELL.Count_3 < 21) {Data_3[CELL.Count_3] = CELL.Value_3 = Value;}					// 측정 횟수 가 20보다 작으면 버퍼에 카운터 듣 적용
						else {Data_3[21] = CELL.Value_3 = Value; for(uint8_t i = 1; i < 21; i++) {Data_3[i] = Data_3[i+1];}}	// 측정 횟수 가 20보다 크면 버퍼를 1칸씩 니려서 저장
						Cell_Desplay(Number);											// 로드셀 무게표시
						Alarm();												// 알람
						CELL.Zero_Buff_3 = 0;											// 로드셀 Zero_Buff_1 초기화
						Timer.Check_Limit_Timer_3 = 0;										// 무게측정 제한 시간 초기화 
					}
				}
				OK_Count = 0;					// 측정체중 OK Count 초기화
			}

			if     (Number == 1) {OK_Count_1 = OK_Count; CELL.Old_Value_1 = Old_Value; CELL.Value_1 = Value; Cell_Desplay(Number);}		// 1번 로드셀 이면 OK_Count 저장
			else if(Number == 2) {OK_Count_2 = OK_Count; CELL.Old_Value_2 = Old_Value; CELL.Value_2 = Value; Cell_Desplay(Number);}		// 2번 로드셀 이면 OK_Count 저장
			else if(Number == 3) {OK_Count_3 = OK_Count; CELL.Old_Value_3 = Old_Value; CELL.Value_3 = Value; Cell_Desplay(Number);}		// 3번 로드셀 이면 OK_Count 저장
		}
		else	// 이전체중이 측정체중과 인증체중(50g)을 더한값과 같거나 작고 인증체중(50g)을 뺀값과 같거나 크면서 이전체중과 같으면 이 아니면
		{
			Old_Value = Value;					// 이전체중에 측정체중 저장
			if     (Number == 1) {CELL.Old_Value_1 = Old_Value; CELL.Value_1 = Value; Cell_Desplay(Number); Zero_Count_1 = 0;}
			else if(Number == 2) {CELL.Old_Value_2 = Old_Value; CELL.Value_2 = Value; Cell_Desplay(Number); Zero_Count_2 = 0;}
			else if(Number == 3) {CELL.Old_Value_3 = Old_Value; CELL.Value_3 = Value; Cell_Desplay(Number); Zero_Count_3 = 0;}
			// 1, 2, 3번 로드셀 이면 이전측정체중 저장, 체중표시, Zero_Count 초기화
		}
	}
	else 									// 측정체중이 없면
	{
		if(Number == 1)							// 1번 로드셀 이면
		{
			...
			if(Zero_Count_1 > 5) { Cell_Desplay(Number); CELL.Zero_Buff_1 = 0; Zero_Count_1 = 0; CELL.Old_Value_1 = 0; OK_Count_1 = 0;}
		...
		else if(Number == 2)						// 2번 로드셀 이면
		{
			...
			if(Zero_Count_2 > 5) {Cell_Desplay(Number); CELL.Zero_Buff_2 = 0; Zero_Count_2 = 0; CELL.Old_Value_2 = 0; OK_Count_2 = 0;}
		...
		else if(Number == 3)						// 2번 로드셀 이면
		{
			...
			if(Zero_Count_3 > 5) {Cell_Desplay(Number); CELL.Zero_Buff_3 = 0; Zero_Count_3 = 0; CELL.Old_Value_3 = 0; OK_Count_3 = 0;}
...
//-------------------------------------------------------------------------------------------------
	...
	for(uint8_t i = 20; i > 0; i--)						// 1부터 20까지 20번 반복
	{
		if(Number == 1)							// 1번 로드셀 이면
		{
			if     (CELL.Count_1 > 20) {Count = CELL.Count_1-20+i;}	// Count_1 이 20 보다 크거나 같으면 Count_1을 i 만큼 증가 하고
		...
		{
			if     (CELL.Count_2 > 20){Count = CELL.Count_2-20+i;}	// Count_2 이 20 보다 크거나 같으면 Count_2을 i 만큼 증가 하고
		...
		{
			if     (CELL.Count_3 > 20) {Count = CELL.Count_3-20+i;}	// Count_3 이 20 보다 크거나 같으면 Count_3을 i 만큼 증가 하고
...
{
	CELL.Proportion_1 = (Cell_RO_1 / 10000000 / 4 * 128 / 0.5 * 8388608 / (Cell_Capa_1 * 1000)) / Cell_CNT_1;
	CELL.Proportion_2 = (Cell_RO_2 / 10000000 / 4 * 128 / 0.5 * 8388608 / (Cell_Capa_2 * 1000)) / Cell_CNT_2;
	CELL.Proportion_3 = (Cell_RO_3 / 10000000 / 4 * 128 / 0.5 * 8388608 / (Cell_Capa_3 * 1000)) / Cell_CNT_3;
//	로드셀 비율 = (RO 총합 / 10000000(mV) / ADC 입력전압 X 게인 / 0.5 * ADC Bit/ CAPA 총합 / 로드셀갯수)
}
...
void Var_Init(void)								// 변수 및 로드셀 초기화
{
	...
	Cell_Zero_Set(1);							// 1번 로드셀 0 값 찾기
	_delay_ms(25);
	Cell_Zero_Set(2);							// 2번 로드셀 0 값 찾기
	_delay_ms(25);
	Cell_Zero_Set(3);							// 3번 로드셀 0 값 찾기
	_delay_ms(25);
	Alarm();								// 알람(비프 3번)
...
//-------------------------------------------------------------------------------------------------
	...
	printf("\r\n");
	printf("//=================================================//\r\n");
	printf("//===  Product Name : Pig Scales 1 V%d.%d(SC-500) ===//\r\n",FW_VER / 10, FW_VER % 10);	// ProductName
...
//-------------------------------------------------------------------------------------------------
	...
	Read_eeprom();								// RTC가 설정되어 있는지 확인 하기위해 EEPROM Data 읽기
//-------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------
	while(1)
	{
		...
		else	      {FND_Value_Data(0, 0, 12); _delay_ms(25);}	// FND 0 FND_OFF

		if(Cell_ID_2) {Check_Value(2);}					// 2번 로드셀의 ID가 설정되어 있으면 체중측정 표시
		else	      {FND_Value_Data(1, 1, 12); _delay_ms(25);}	// FND 1 FND_OFF

		if(Cell_ID_3) {Check_Value(3);}					// 3번 로드셀의 ID가 설정되어 있으면 체중측정 표시
		else	      {FND_Value_Data(2, 2, 12); _delay_ms(25);}	// FND 2 FND_OFF
