...
void Check_Value(void)								// 로드셀 체중측정
{
	...
	uint32_t Old_Value = CELL.Old_Value;					// 로드셀 Old_Value 값
 	...
 	uint8_t cnt_2 = 15;							// OK_Count 갯수
//-------------------------------------------------------------------------------------------------
	if(Value)								// 측정체중이 있으면
	{
 		if((Old_Value <= Value + Admit_Weight) & (Old_Value >= Value - Admit_Weight))
		{								// 이전체중이 측정체중과 인증체중(10x10g)을 더한값과 같거나 작고 인증체중(10x10g)을 뺀값과 같거나 크면서 이전체중과 같으면
...
//-------------------------------------------------------------------------------------------------
					...
					SET.Save = 0;
					Alarm();
					printf("OK_Count_1 %d, OK_Count_2 %d \r\n", OK_Count_1, OK_Count_2);
					printf("Value %ld \r\n", Old_Value);
		...
		else
		{
			...
			{							// 이전체중이 측정체중과 인증체중((10+5)x10g)을 더한값과 같거나 작고 인증체중((10+5)x10g)을 뺀값과 같거나 크면서 이전체중과 같으면
