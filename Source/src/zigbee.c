...
void ZigBee_Write_id(void)							// ZigBee PanID 저장
{
	...
	uint8_t zbid_retry = 0;							// 재시도 횟수 추가
...
//-------------------------------------------------------------------------------------------------
		for(uint8_t cnt = 0; cnt < RX3_SIZE - 2; cnt++)			// RX3_SIZE 만큼 반복
		{
		...
		wdt_reset();							// 워치독 리셋
		if(zbid_retry++ > 10) {break;}					// 10회 초과 시 강제 탈출
...
void ZigBee_Write_ch(void)							// ZigBee 채널 저장
{
	...
	uint8_t zbch_retry = 0;							// 재시도 횟수 추가
...
//-------------------------------------------------------------------------------------------------
		...
		wdt_reset();							// 워치독 리셋
		if(zbch_retry++ > 10) {break;}					// 10회 초과 시 강제 탈출
...
void ZigBee_Read(void)								// ZigBee 설정값 읽기
{
	...
	uint8_t zbrd_retry = 0;							// 재시도 횟수 추가
...
//-------------------------------------------------------------------------------------------------
		...
		wdt_reset();							// 워치독 리셋
	if(zbrd_retry++ > 10) {break;}						// 10회 초과 시 강제 탈출
