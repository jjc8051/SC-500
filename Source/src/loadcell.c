...
int32_t Cell_Data_Read(void)							// 로드셀 Data 읽기, HX711 제어
{
	...
	uint16_t timeout = 0;							// 타임아웃 카운터

	SCK_LOW_1;								// HX711 SCk LOW
	while(DO1)
	{
		_delay_us(25);							// 25us 대기
		wdt_reset();							// WDT 리셋
		if(++timeout > 2000)						// 500ms (25us × 2000) 초과 시
		{
			return 0;						// HX711 응답 없음, 0 반환
		}
	}									// HX711 SDA가 HIGH 이면 대기
	...
	Value = Value ^ 0x400000;						// 전체 Data 의 2의 보수 값 구하기 (Value ^ 0x400000은 버그가 아니라 이 하드웨어에 맞는 의도적인 오프셋 처리)
...
//-------------------------------------------------------------------------------------------------
	if(Value > 0)								// 무게 Data 가 있으면
	{
		...
		if(ValSum < 0 || ValSum > CAPA)					// ValSum 이 0 보다 작거나 CAPA 보다 크면 초기화 카운터
		{
			...
			if(Value_Count > 5)					// Value_Count 가 초기화 값 보다 크면
			{
...
void Interval_Set(void)								// Interval 설정
{
	...
	uint32_t Old_Value;							// 로드셀 Old_Value 값
